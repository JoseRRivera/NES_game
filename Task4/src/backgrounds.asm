...
.export load_attributes
.proc load_attributes

    ; Calculate the starting address of the attribute table
    LDA #$23      ; Example starting nametable address (change as needed)
    STA PPUADDR   ; Set the high byte of PPU address
    LDA #$C0      ; Example attribute table offset (change as needed)
    STA PPUADDR   ; Set the low byte of PPU address

    ; Iterate through each byte in the attribute table
    LDX #$00      ; Initialize X register as loop counter
    LDY #$00      ; Initialize Y register as attribute value counter
LOOP:
    ; Write the attribute value to the attribute table
    ;LDA attribute_table,X   ; Example attribute value (change as needed)
    STA PPUDATA

    ; Increment loop counter
    INX
    CPX #64    ; Check if 32 bytes (attribute table size) have been written
    BNE LOOP   ; Loop back if not yet finished
